...
const copy = {
  pt: {
    ...
    learnMoreMsg: (nome: string) => `Olá! Quero saber mais sobre: ${nome}.`,
  },
  en: {
    buy: "Buy",
    processing: "Processing…",
    learnMore: "Learn more",
    learnMoreMsg: (nome: string) => `Hello! I'd like to know more about: ${nome}.`,
  },
} satisfies Record<Locale, unknown>;

function CardProduto({ produto, locale }: { produto: Produto; locale: Locale }) {
  const t = copy[locale];
  ...
  const nome = produto.nome[locale];
  const msgSaibaMais = t.learnMoreMsg(nome);
...
export function Loja({ produtos, locale }: { produtos: Produto[]; locale: Locale }) {
  return (
